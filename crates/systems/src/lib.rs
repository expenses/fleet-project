...
type TargettingFilter<T> = HasComponents<(ComponentFilter<T>, Not<ComponentFilter<Targetting>>)>;

#[system]
pub fn choose_enemy_target(
    friendlies: &mut Query<(Entity, &Position, &ModelId), TargettingFilter<Friendly>>,
    enemies: &mut Query<(Entity, &Position, &ModelId), TargettingFilter<Enemy>>,
    world: &legion::world::SubWorld,
    command_buffer: &mut legion::systems::CommandBuffer,
) {
    friendlies.for_each(world, |(entity, pos, _)| {
        let target = enemies
            .iter(world)
            .map(|(target_entity, target_pos, _)| (target_entity, (target_pos.0 - pos.0).mag_sq()))
            .min_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    ...
    enemies.for_each(world, |(entity, pos, _)| {
